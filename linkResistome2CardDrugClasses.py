...
"""This script takes gene mapping files as produced by rgi bwt and derives a secondary annotation that uses terms more common in the resistome world.
...
#mapping terms from the Antibiotic resistance Ontology to terms that are used in Resistome:
...
def getTally(sid):
    ...
    for i,row in gene.iterrows():
        ...
        if len(set(equiResistome)) <=1 and len(drugclasses)> 1:
