...
data = []
nameMatchTable = []
nameMatchTableHeader = ['RM', 'CARDcons', 'drugclass', 'drugclass2'] + ['match %i'%i for i in range(1,matches2rep + 1)] + ['lcs']
...
for idx, row in primers.iterrows():
    ...
    data.append([row["Suggestion "], row["Gene target"], row['Unnamed: 3'], row['Unnamed: 4'], len(matches) ] + matches[:10] + [nameCons100, lcs])

    if len(matches)>-1: ## -1: everything
...
ndf.to_csv('primerCrossmatch2all2.csv') 
